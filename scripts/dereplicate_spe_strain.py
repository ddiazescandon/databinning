...
def go(ax, num_rows):
    ...
    for k in range(4):
        i, j = k//2, k%2
        if i==0 and j== 0:
            ax[i, j].bar(0 - .2, 111, width=.2, color=cat_colors[0],
                   label='Short_co')
            ...
            ax[i, j].bar(0 - .2, 111, width=.2, color=cat_colors[0],
                   label='Short_co')
            ax[i, j].bar(0, 152, width=.2, color=cat_colors[1],
                   label='Short_single')
            ...
            ax[i, j].bar(0 - .2, 8, width=.2, color=cat_colors[0],
                   label='Short_co')
...
def main():
    ...
    fig.savefig('./no_of_spe_str_checkm2_comebin.pdf', dpi=300, format='pdf', bbox_inches='tight')
    plt.close()
