...
def go(ax, num_rows):
    ...
    for i in range(2):
        for j in range(2):
            if i==0 and j==0:
                ax[i, j].bar(0 - .2, 70, width=.2, color=cat_colors[0],
                       label='Short_co')
                ...
                ax[i, j].bar(2 + .1, 193, width=.2, color=cat_colors[6],
                       label='Hybrid_multi')
            if i == 0 and j == 1:
                ax[i, j].bar(0 - .2, 7, width=.2, color=cat_colors[0],
                             label='Short_co')
                ...
                ax[i, j].bar(0 - .2, 382, width=.2, color=cat_colors[0],
                       label='Short_co')
                ...
                ax[i, j].bar(0 - .2, 253, width=.2, color=cat_colors[0],
                             label='Short_co')
...
def main():
    ...
    fig.savefig('./bgc_and_arg.pdf', dpi=300, format='pdf', bbox_inches='tight')
    plt.close()
